...
test('server bootstrap claim survives downloads and ends only after a terminal report', async () => {
  ...
  expect(script).not.toContain('Description=Update Gravit server pack')
  expect(script).not.toContain('server-pack.tar.gz')
  expect(script).not.toContain('server-pack-files')
