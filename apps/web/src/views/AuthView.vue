    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <Card>
        ...
        <CardContent class="space-y-4">
          ...
          <div v-if="recipeId === 'discord'" class="flex items-center justify-between gap-4 rounded-md border bg-muted/20 p-4">
            <div>
              <p class="text-sm font-medium">Built-in texture provider</p>
              <p class="text-xs text-muted-foreground">Use skins uploaded by players in the public cabinet.</p>
            </div>
            <Switch v-model="builtInTextureProvider" />
          </div>
...
const builtInTextureProvider = ref(false)
...
watch(
  ...
  (value) => {
    ...
    builtInTextureProvider.value = value.coreType === 'discordauthsystem' && value.textureProvider?.type === 'request'
...
const {
  ...
  mutationFn: () => {
    const body: Record<string, unknown> = {
      ...
      textureProvider: recipeId.value === 'discord' && builtInTextureProvider.value
        ? { type: 'request' }
        : { type: 'void' },
